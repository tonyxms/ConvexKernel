...
### Satellite example for O_2, input dimension is 14, n_train=200
B=50
...
train <- read.table("CD_GRACE_1000_O2.dat")
test <- read.table("CD_GRACE_100_O2.dat")
...
#write.csv(dim_10,file='Satellite_o2_200.csv')
save.image('result/Satellite_o2_200.RData')

rm(list=ls())

source('header.R')

### Satellite example for O_2, input dimension is 14, n_train=500
B=20
RMSPE_sd <- data.frame(laGP=numeric(B),Opt_kernel=numeric(B),mlegp=numeric(B),MRFA=numeric(B))
theta=seq(from=1,to=9,length=5)
theta=c(theta*10^-2,theta*10^-1,theta,theta*10,theta*10^2)
...
#write.csv(dim_10,file='Satellite_o2_500.csv')
save.image('result/Satellite_o2_500.RData')