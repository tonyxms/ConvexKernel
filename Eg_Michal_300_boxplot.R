...
### Michalewicz example for 6 active dimensions, input dimension p=10, n_train=300
B=50
...
for (k in 1:B){
  ...
  x <- maximinLHS(n, k=p)
  y <- michal_6(x[,1:6])+runif(n,min=-0.02,max=0.02) # add noise
...
#ggsave('Michal_10dim_300_50.pdf')
...
#write.csv(dim_10,file='Michal_10dim_300_50.csv')
save.image('result/Michal_10dim_300_50.RData')

rm(list=ls())

source('header.R')

### Michalewicz example for 6 active dimensions, input dimension p=20, n_train=300
B=50
RMSPE_sd <- data.frame(laGP=numeric(B),Opt_kernel=numeric(B),mlegp=numeric(B),MRFA=numeric(B))
theta=seq(from=1,to=9,length=5)
theta=c(theta*10^-2,theta*10^-1,theta,theta*10,theta*10^2)
...
#ggsave('Michal_20dim_300_50.pdf')
...
#write.csv(dim_10,file='Michal_20dim_300_50.csv')
save.image('result/Michal_20dim_300_50.RData')

rm(list=ls())

source('header.R')

### Michalewicz example for 6 active dimensions, input dimension p=60, n_train=300
B=50
RMSPE_sd <- data.frame(laGP=numeric(B),Opt_kernel=numeric(B),mlegp=numeric(B),MRFA=numeric(B))
theta=seq(from=1,to=9,length=5)
theta=c(theta*10^-2,theta*10^-1,theta,theta*10,theta*10^2)
...
#ggsave('Michal_60dim_300_50.pdf')
...
#write.csv(dim_10,file='Michal_60dim_300_50.csv')
save.image('result/Michal_60dim_300_50.RData')
