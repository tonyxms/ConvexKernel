...
### Satellite example for He, input dimension is 14, n_train=200
B=50
...
train <- read.table("CD_GRACE_1000_He.dat")
test <- read.table("CD_GRACE_100_He.dat")
...
#write.csv(dim_10,file='Satellite_he_200.csv')
save.image('result/Satellite_he_200.RData')

rm(list=ls())

source('header.R')

### Satellite example for He, input dimension is 14, n_train=500
B=20
RMSPE_sd <- data.frame(laGP=numeric(B),Opt_kernel=numeric(B),mlegp=numeric(B),MRFA=numeric(B))
theta=seq(from=1,to=9,length=5)
theta=c(theta*10^-2,theta*10^-1,theta,theta*10,theta*10^2)
...
#write.csv(dim_10,file='Satellite_he_500.csv')
save.image('result/Satellite_he_500.RData')
